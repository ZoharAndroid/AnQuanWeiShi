...
public class SmsTelActivity extends Activity {
	...
	public static final String INTENTINTERFACE = "phoneNumber";
	...
	private View popupView;// 添加的popupView界面

	private PopupWindow popup;// popup属性
	
	private ScaleAnimation scale;//动画
		...
		new Thread() {
			public void run() {
				...
				BlacklistNameDB blacklistDB = BlacklistNameDB
						.getInstance(SmsTelActivity.this);
				// 然后读取数据库中的内容
	...
	public void initPopupView() {
		popupView = View.inflate(SmsTelActivity.this, R.layout.popup_smstel,
				null);
		popup  = new PopupWindow(popupView, -2, -2);
		//设置动画
		popup.setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
		scale = new ScaleAnimation(1, 1, 0, 1, Animation.RELATIVE_TO_SELF, 0, Animation.RELATIVE_TO_SELF, 0);
		scale.setDuration(300);
		popupView.setAnimation(scale);

		// 找到popupView中的控件
		TextView tv_shoudongAdd = (TextView) popupView
				.findViewById(R.id.popup_smstel_shoudongAdd);
		TextView tv_smsAdd = (TextView) popupView
				.findViewById(R.id.popup_smstel_smsAdd);
		TextView tv_phoneAdd = (TextView) popupView
				.findViewById(R.id.popup_smstel_phoneAdd);
		TextView tv_contactsAdd = (TextView) popupView
				.findViewById(R.id.popup_smstel_contactsAdd);

		OnClickListener listemer = new OnClickListener() {

			@Override
			public void onClick(View v) {
				switch (v.getId()) {
				case R.id.popup_smstel_shoudongAdd:
					addBlacklist("");//手动添加
					break;
				case R.id.popup_smstel_smsAdd://短信列表添加
					Intent smsIntent = new Intent(SmsTelActivity.this, SmsListAddBlackActivity.class);
					startActivityForResult(smsIntent, 0);
					break;
				case R.id.popup_smstel_phoneAdd://通话记录添加
					Intent telphoneIntent = new Intent(SmsTelActivity.this, TelphoneListAddBlackActivity.class);
					startActivityForResult(telphoneIntent, 0);
					break;
				case R.id.popup_smstel_contactsAdd://联系人添加
					Intent contactsIntent = new Intent(SmsTelActivity.this, ListContactsActvity.class);
					startActivityForResult(contactsIntent, 0);
					break;
				default:
					break;
				}
		...
		tv_shoudongAdd.setOnClickListener(listemer);
		tv_smsAdd.setOnClickListener(listemer);
		tv_phoneAdd.setOnClickListener(listemer);
		tv_contactsAdd.setOnClickListener(listemer);
	}
	 ...
	 * 
	 */
	public void addBlacklistPopup(View view) {
		if (popup != null && popup.isShowing()) {
			// 如果popup不为空并且正在显示
			// 撤销popup
			popup.dismiss();
		} else {
			popup.showAtLocation(view, Gravity.RIGHT | Gravity.TOP, 15, 75);
			//开始动画
	 ...
	 * @param view
	 */
	public void addBlacklist(String phoneNumber) {
		// 一点击，就弹出对话框，进行添加
		AlertDialog.Builder blackDialog = new AlertDialog.Builder(
				SmsTelActivity.this);
		View dialogView = View.inflate(SmsTelActivity.this,
				R.layout.setblacklist_dialog_smstel, null);

		// 找到该对话框中的控件
		final EditText et_setBlack = (EditText) dialogView
				.findViewById(R.id.et_dialog_smstel_blackNumber);
		final CheckBox cb_setSms = (CheckBox) dialogView
				.findViewById(R.id.cb_dialog_smstel_setSms);
		final CheckBox cb_setPhone = (CheckBox) dialogView
				.findViewById(R.id.cb_dialog_smstel_setPhone);

		et_setBlack.setText(phoneNumber);
		
		Button et_sureSetBlack = (Button) dialogView
				.findViewById(R.id.bt_dialog_smstel_sureSetBlack);
		/**
		 * 按键：确定设置黑名单
		 */
		et_sureSetBlack.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// 先判断用户是否输入了黑名单号码
				String blacklistNumber = et_setBlack.getText().toString()
						.trim();
				...
				// 保存数据
				BlacklistMember listMember = new BlacklistMember(
						blacklistNumber, mode);
				blackDB = BlacklistNameDB.getInstance(SmsTelActivity.this);
		 ...
		 * 按键：取消黑名单设置
		 */
		Button et_cancelSetBlack = (Button) dialogView
				.findViewById(R.id.bt_dialog_smstel_cancelSetBlack);
	...
	private class MyAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			if (convertView == null) {
				convertView = View.inflate(SmsTelActivity.this,
						R.layout.item_smstel, null);
				holder = new ViewHolder();
				// 然后获得该界面的布局控件
				holder.tv_blacknumber = (TextView) convertView
						.findViewById(R.id.tv_item_smstel_number);
				holder.tv_mode = (TextView) convertView
						.findViewById(R.id.tv_item_smstel_mode);
				holder.iv_deleteIcon = (ImageView) convertView
						.findViewById(R.id.iv_item_smstel_deleteicon);
			...
			holder.iv_deleteIcon.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					blackDB = BlacklistNameDB.getInstance(SmsTelActivity.this);
					AlertDialog.Builder notineDialog = new AlertDialog.Builder(
							SmsTelActivity.this);
					notineDialog.setTitle("提醒：");
					notineDialog.setMessage("确认删除:"
							+ members.get(position).getBlacklistNumber() + "?");
					notineDialog.setPositiveButton("确定",
							new DialogInterface.OnClickListener() {

								@Override
								public void onClick(DialogInterface dialog,
										int which) {
									blackDB.removeBlacklist(members
											.get(position));// 数据库中清除该内容
									// 清空容器
									members.clear();
									// 重新加载容器
									members = blackDB.queryBlacklist();
